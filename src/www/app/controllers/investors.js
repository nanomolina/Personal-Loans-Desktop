...
InvestorCtrl.$inject = ['Investor', '$scope', '$mdToast', '$mdDialog', '$routeParams'];

function InvestorCtrl(Investor, $scope, $mdToast, $mdDialog, $routeParams) {
    var vm = this;

    $scope.master.toolbar = {title: 'Inversores', icon: 'content/img/business/diagram.svg'}
    vm.data = {
      first_name: '',
      last_name: '',
      alias: '',
    ...
    function editInvestor() {
      if ($scope.investorDetailForm.$valid) {
        ...
        Investor.update(assoc_id, inv_id, vm.data_edit)
        .then(function(response) {
          vm.update_loading = false;
    ...
    function removeInvestor() {
      var assoc_id = $routeParams.associationId;
      var inv_id = vm.selected_investor.id;
      Investor.remove(assoc_id, inv_id)
      .then(function(response) {
